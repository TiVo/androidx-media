...
public class HlsMediaChunkTest {
  ...
  private static final String PLAYLIST_NON_INDEPENDENT_SEGMENTS = "media/m3u8/media_playlist";
  ...
  private static final Format IFRAME_FORMAT =
      BASE_VIDEO_FORMAT.buildUpon().setRoleFlags(C.ROLE_FLAG_TRICK_PLAY).build();
  ...
  public void test_shouldSpliceIn_PreviousLoaded_SamePlaylist() {
    ...
    HlsMediaChunk previousChunk =
        createTestHlsMediaChunk(BASE_VIDEO_FORMAT, segmentBaseHolder, PLAYLIST_URI, true);
    previousChunk.setLoadCompleted();
    boolean result =
        HlsMediaChunk.shouldSpliceIn(
            previousChunk, PLAYLIST_URI, playlist, BASE_VIDEO_FORMAT, segmentBaseHolder, 0);
  ...
  public void test_shouldSpliceIn_NotIndependent_DifferentPlaylist() {
    ...
    HlsMediaChunk previousChunk =
        createTestHlsMediaChunk(BASE_VIDEO_FORMAT, previousSegmentBaseHolder, variant1, true);

    Uri variant2 = Uri.parse("http://example.com/variant2.m3u8");
    HlsMediaPlaylist nonIndependentPlaylist =
        HlsTestUtils.getHlsMediaPlaylist(PLAYLIST_NON_INDEPENDENT_SEGMENTS, variant2);
    ...
    assertThat(previousSegmentBaseHolder.segmentBase.relativeStartTimeUs)
        .isEqualTo(0); // inputs assertions
    assertThat(
            nextSegmentBaseHolder.segmentBase.relativeStartTimeUs
                + nextSegmentBaseHolder.segmentBase.durationUs)
        .isGreaterThan(0);
    boolean result =
        HlsMediaChunk.shouldSpliceIn(
            previousChunk,
            variant2,
            nonIndependentPlaylist,
            BASE_VIDEO_FORMAT,
  ...
  public void test_shouldSpliceIn_Independent_DifferentPlaylist() {
    ...
    // Switch to inependent segment playlist requires splice-in, only if the start of the next
    // segment is
    ...
    boolean result =
        HlsMediaChunk.shouldSpliceIn(
            previousChunk, variant2, playlist, BASE_VIDEO_FORMAT, nextSegmentBaseHolder, 0);
    assertThat(result).isTrue();

    nextSegmentBaseHolder = new HlsChunkSource.SegmentBaseHolder(playlist.segments.get(1), 1, 0);
    result =
        HlsMediaChunk.shouldSpliceIn(
            previousChunk, variant2, playlist, BASE_VIDEO_FORMAT, nextSegmentBaseHolder, 0);
    assertThat(result).isFalse();
  ...
  public void test_shouldSpliceIn_SegmentParts() {
    ...
    HlsMediaPlaylist hlsMediaPlaylist =
        HlsTestUtils.getHlsMediaPlaylist(PLAYLIST_INDEPENDENT_PART, variant2);
    ...
    boolean result =
        HlsMediaChunk.shouldSpliceIn(
            previousChunk, variant2, hlsMediaPlaylist, BASE_VIDEO_FORMAT, nextSegmentBaseHolder, 0);

    assertThat(result).isFalse();

    // Additional Parts must be themselves independent, regardless of the independence of the
    // playlist
    nextSegmentBaseHolder =
  ...
  public void test_shouldSpliceIn_IntraTrickPlay() {
    ...
    HlsMediaPlaylist hlsMediaPlaylist4s =
        HlsTestUtils.getHlsMediaPlaylist(PLAYLIST_IFRAME_4s, variant4);
    HlsChunkSource.SegmentBaseHolder previousSegmentBaseHolder =
        new HlsChunkSource.SegmentBaseHolder(hlsMediaPlaylist4s.segments.get(0), 0, 0);
    HlsMediaChunk previousChunk =
        createTestHlsMediaChunk(IFRAME_FORMAT, previousSegmentBaseHolder, variant4, true);

    Uri variant2 = Uri.parse("http://example.com/iframe2.m3u8");
    HlsMediaPlaylist hlsMediaPlaylist2s =
        HlsTestUtils.getHlsMediaPlaylist(PLAYLIST_IFRAME_2s, variant2);
    HlsChunkSource.SegmentBaseHolder nextSegmentBaseHolder =
        new HlsChunkSource.SegmentBaseHolder(hlsMediaPlaylist2s.segments.get(1), 0, 0);
    boolean result =
        HlsMediaChunk.shouldSpliceIn(
            previousChunk, variant2, hlsMediaPlaylist2s, IFRAME_FORMAT, nextSegmentBaseHolder, 0);
  ...
  public void test_shouldSpliceIn_NonTrickPlay_To_TrickPlay() {
    ...
    HlsChunkSource.SegmentBaseHolder previousSegmentBaseHolder =
        new HlsChunkSource.SegmentBaseHolder(playlist.segments.get(0), 0, 0);
    HlsMediaChunk previousChunk =
        createTestHlsMediaChunk(BASE_VIDEO_FORMAT, previousSegmentBaseHolder, PLAYLIST_URI, true);
