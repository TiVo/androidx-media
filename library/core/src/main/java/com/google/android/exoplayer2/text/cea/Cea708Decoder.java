...
public final class Cea708Decoder extends CeaDecoder {
  ...
  private void printCommandName(int command) {
    ...
    switch (command) {
      ...
      case COMMAND_DF4: commandName = "DefineWindow 4"; break;
  ...
  private static final class CueBuilder {
    ...
    public void setPenLocation(int row, int column) {
      ...
      // Till the support is added, keeping this simple work aorund. This
      // mostly not used but essential to pass the certification.  
      if ((row == 0) && (column == 0)) {
          setPenLocWorkAround = true;
      }
    }
    ...
    public Cea708Cue build() {
      ...
      if (cueString.length() == 0) {
          // Window is not shown when the cueString is empty. Sarnoff stream
          // sync test test requires display of empty window. So, for now
          // simply adding a tranparent space to make window flash. 
          cueString.append('\u00A0');
      }
